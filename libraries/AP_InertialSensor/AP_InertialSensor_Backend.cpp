...
void AP_InertialSensor_Backend::log_gyro_raw(uint8_t instance, const uint64_t sample_us, const Vector3f &raw_gyro, const Vector3f &filtered_gyro)
{
...
#else
    if (should_log_imu_raw()) {
        // Allow configuration with batch sampler options
        if (_imu.batchsampler.has_option(AP_InertialSensor::BatchSampler::BATCH_OPT_PRE_POST_FILTER)) {
            // Both pre and post, offset post instance as batch sampler does
            Write_GYR(instance, sample_us, raw_gyro);
            Write_GYR(instance + _imu._gyro_count, sample_us, filtered_gyro);

        } else if (_imu.batchsampler.has_option(AP_InertialSensor::BatchSampler::BATCH_OPT_POST_FILTER)) {
            // Just post
    ...
    } else if (!_imu.batchsampler.doing_sensor_rate_logging()) {
        _imu.batchsampler.sample(instance, 
                                 AP_InertialSensor::IMU_SENSOR_TYPE_GYRO,
                                 sample_us,
                                 !_imu.batchsampler.doing_post_filter_logging() ? raw_gyro : filtered_gyro);
    }
#endif // AP_INERTIALSENSOR_BATCHSAMPLER_ENABLED
